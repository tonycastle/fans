...
const AddCardForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.post("/api/payments/addcard", { id: User });
  ...
  const handleSubmit = async (event) => {
    ...
    if (result.error) {
      //TODO: if there is a problem display an error dialog

      setCreateCardError(true);
    } else {
      //new card successfully created show some positive feedback and or redirect to displaycards
      history.push("/settings/displaycards");
    }
  };
  ...
  return (
    <div>
      {createCardError && (
        <ErrorDialog
          msg={`Hmmm, that's not supposed to happen, could not create new card.`}
        />
      )}
